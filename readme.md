...
=======
3 不管直接 html、 vue 、angular、 ... 前端没有直接的文件系统,chrome 之前有实现性质新特性 API 可直接关联操作系统的文件系统,(⊙o⊙)…,没必要，先 pass 了.
>>>>>>> b0703a993729947c04401a6f9208f7fc1c84ca01
...
<<<<<<< HEAD
### ~~2 某些地区通过百度地图 JS API 查询出来的经纬度可能不准确~~
...
这个问题开头就提到过，这么多地区，我也不知道哪些 API 查出来的结果不准确，还在思考当中（20201211）
...
之前直接使用百度地图 API 的 GetPoint()接口,会出现偏差特别大的结果,例如顶上截图所示,指定省市区查询镇有直接结果,但从海南跑到了青海(内部有个以"海南"为名的行政).

我找到另外一个接口 LocalSearch(),指定区域内搜索地址,这样以文字搜地址,从效果来看,第一个预留地址与实际预计查询结果最接近.例如顶页问题:

![頂頁問題解決](./notes/pic/頂頁問題解決.png)
...
但以上任意相邻两个的组合或者三者组合"贵州省黔东南苗族侗族自治州剑河县"下搜索"柳川镇",localSearch 返回`状态码`为 2 : `BMAP_STATUS_UNKNOWN_LOCATION`,找不到位置.有点返回就可能是 1 : `BMAP_STATUS_CITY_LIST`,很多市级地区有同名的乡镇,即便已经指定了市级名称)
